...
from ..sandbox import SandboxStats, fetch_sandbox_stats
from ..sandbox.stats import VerifiedIoc
...
def _pretty_print(stats: SandboxStats, show_hashes: bool = False) -> None:
    console = Console()
    frontend_base = _SANDBOX_FRONTEND_URLS.get(stats.sandbox_choice, _SANDBOX_FRONTEND_URLS['eu'])
...
def stats(
    ...
    if pretty:
        _pretty_print(result, show_hashes=hashes)
    else:
        print_json(json.dumps(_to_json_dict(result)), indent=2)
