...
- IOC の高速バルクエンリッチメントのための新しい [`ioc bulk-lookup`](reference/commands.md#banshee-ioc-bulk-lookup) サブコマンドを追加。API 呼び出しごとに最大 1,000 件のインジケーターをバッチ処理し、各インジケーターのリスクスコアとトリガーされたリスクルールを返す。IP、ドメイン、URL、ハッシュ、脆弱性のすべての IOC タイプをサポート。
