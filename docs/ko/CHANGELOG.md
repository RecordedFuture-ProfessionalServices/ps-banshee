...
### Added
- 새로운 [`email enrich`](reference/commands.md#banshee-email-enrich) 서브 명령어를 추가하였습니다. EML 파일을 보강하며, 헤더 IP 및 본문 URL을 추출한 후 위험 점수, 위협 행위자 연관 정보, 악성 코드 링크, 위험 규칙 증거를 포함한 Recorded Future 인텔리전스를 반환합니다.
