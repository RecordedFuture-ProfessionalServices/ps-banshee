...
## Solution
Enrich EML files directly in PS Banshee using [`banshee email enrich`](../../reference/commands.md#banshee-email-enrich). The command automatically parses the email, extracts IP addresses from the message headers and URLs from the body, and enriches each indicator with Recorded Future threat intelligence — returning risk scores, threat actor associations, malware links, and triggered risk rules in a single step.
