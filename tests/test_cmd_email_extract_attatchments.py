...
class TestExtractAttachments:
    def test_extracts_attachment_and_writes_encrypted_zip(self, tmp_path):
        ...
        result = extract_attachments(DEMO_EML, zip_path)
    ...
    def test_zip_requires_password(self, tmp_path):
        zip_path = tmp_path / 'out.zip'
        extract_attachments(DEMO_EML, zip_path)
    ...
    def test_no_attachments_returns_empty_list(self, tmp_path):
        ...
        result = extract_attachments(SIMPLE_EML, zip_path)
...
class TestSandboxAttachments:
    @patch('banshee.email.attachments_sandbox.SandboxMgr')
    def test_no_attachments_json_output(self, mock_sandbox_cls, tmp_path, capsys):
        sandbox_attachments(str(SIMPLE_EML), tmp_path, pretty=False)
    ...
    def test_no_attachments_pretty_output(self, mock_sandbox_cls, tmp_path, capsys):
        sandbox_attachments(str(SIMPLE_EML), tmp_path, pretty=True)

        captured = capsys.readouterr()
        assert f'No files were extracted from {SIMPLE_EML}' in captured.out
        mock_sandbox_cls.assert_not_called()

    @patch('banshee.email.attachments_sandbox.time.sleep')
    @patch('banshee.email.attachments_sandbox.time.monotonic')
    @patch('banshee.email.attachments_sandbox.SandboxMgr')
    def test_polling_timeout_reports_failure(
        ...
        sandbox_attachments(str(DEMO_EML), tmp_path, pretty=True)
...
class TestCmdEmailExtractAttachments:
    @patch('banshee.commands.cmd_email.sandbox_attachments')
    def test_invokes_sandbox_attachments_with_defaults(self, mock_sandbox, tmp_path):
        result = runner.invoke(app, ['extract-attachments', str(DEMO_EML), '-z', str(tmp_path)])

        assert result.exit_code == 0
        mock_sandbox.assert_called_once_with(str(DEMO_EML), tmp_path, False)

    @patch('banshee.commands.cmd_email.sandbox_attachments')
    def test_invokes_sandbox_attachments_with_pretty(self, mock_sandbox, tmp_path):

        result = runner.invoke(
            app, ['extract-attachments', str(DEMO_EML), '-z', str(tmp_path), '-p']
        )
    ...
    def test_help_available(self):
        result = runner.invoke(app, ['extract-attachments', '--help'])
