...
class TestCmdSandboxReportBehavioral:
    ...
    def test_behavioral_default(self, mock_fetch):
        ...
        mock_fetch.assert_called_once_with(
            '251114-py23jaavtp', pretty=False, wait=False, full_cmd=False
        )
    ...
    def test_behavioral_pretty_long_flag(self, mock_fetch):
        ...
        assert result.exit_code == 0
        mock_fetch.assert_called_once_with(
            '251114-py23jaavtp', pretty=True, wait=False, full_cmd=False
        )
    ...
    def test_behavioral_wait_long_flag(self, mock_fetch):
        ...
        assert result.exit_code == 0
        mock_fetch.assert_called_once_with(
            '251114-py23jaavtp', pretty=False, wait=True, full_cmd=False
        )
    ...
    @patch('banshee.commands.cmd_sandbox.fetch_behavioral_reports')
    def test_behavioral_full_cmd_flag(self, mock_fetch):
        result = runner.invoke(app, ['report', 'behavioral', '251114-py23jaavtp', '--full-cmd'])
        assert result.exit_code == 0
        mock_fetch.assert_called_once_with(
            '251114-py23jaavtp', pretty=False, wait=False, full_cmd=True
        )
    ...
    def test_behavioral_help_available(self):
        ...
        assert '--full-cmd' in result.output
